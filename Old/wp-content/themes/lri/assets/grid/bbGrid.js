...
(function () {
    ...
    _.extend(bbGrid.PagerView.prototype, Backbone.View.prototype, {
      ...
      template: _.template(
            '<div class="row"><div class="col-xs-6"><div class="span bbGrid-pager">\
                <ul class="pagination">\
                    <li<%if (page <= 1) {%> class="active"<%}%>>\
                        <a class="first"><span class="glyphicon glyphicon-step-backward"></span></a>\
                    </li>\
                    <li <%if (page <= 1) {%> class="active"<%}%>>\
                        <a class="left"><span class="glyphicon glyphicon-backward"></span></a>\
                    </li>\
                    <li>\
                    ...
                    <li<%if (page === cntpages || page === 0) {%> class="active"<%}%>>\
                        <a class="right"><span class="glyphicon glyphicon-forward"></span></a>\
                    </li>\
                    <li<%if (page === cntpages || page === 0) {%> class="active"<%}%>>\
                        <a class="last"><span class="glyphicon glyphicon-step-forward"></span></a>\
                    </li>\
                </ul>\
                </div></div>\
            <% if (rowlist) {%>\
                <div class="col-xs-6">\
                <div class="bbGrid-pager-rowlist-label"><%=dict.rowsOnPage%>:\
    ...
    bbGrid.FilterView = function (options) {
        this.events = {
            ...
            'click .filterValue': 'onRemoveFilter',
            'click .filterClear': 'onRemoveAllFilters',
        };
        if (options.view.loadDynamic) {
    ...
    _.extend(bbGrid.FilterView.prototype, Backbone.View.prototype, {
            ...
            <%_.each(cols, function (col) {%>\
                <td>\
                    ...
                    <option value=""><%=dict.all%></option>\
                        ...
                        <% if(columnFilters != undefined && columnFilters[col.name] != undefined){%>\
                          <div style="margin: 5px 0 5px 0">\
                          <% _.each(columnFilters[col.name], function(filter){%>\
                          <span style="margin: 3px;"><a class="filterValue" data-colname="<%=col.name%>" href="#"><%=filter%></a></span>\
                        <%})%>\
                          <%if(columnFilters[col.name].length > 0){%>\
                            <span class="filterClear label label-danger" data-colname="<%=col.name%>">Clear all</span>\
                          <%}%>\
                        <%}%>\
                    </div>\
                    <%}%>\
        ...
        initialize: function (options) {
            ...
            options.view.columnFilters = {};
            this.options = {};
        },
        onRemoveAllFilters: function(event){
            var self = this;

            var name = $(event.target).data('colname');

            if(self.view.columnFilters != undefined && self.view.columnFilters.hasOwnProperty(name)){
                self.view.columnFilters[name] = [];
            }

            if(self.view.filterOptions != undefined && self.view.columnFilters.hasOwnProperty(name)){
            ...
            this.view.trigger('filter', {silent: true});
        },
        onRemoveFilter: function(event){
            ...
            if(self.view.columnFilters != undefined && self.view.columnFilters.hasOwnProperty(name)){
                self.view.columnFilters[name] = _.without(self.view.columnFilters[name], value);
            }
        ...
        onFilter: function () {
            ...
            _.each($('*[name=filter]', this.$el), function (el) {
                ...
                if(self.view.columnFilters[name] == undefined){
                    self.view.columnFilters[name] = [];
                }

                if(text.length > 0 && !_.contains(self.view.columnFilters[name], text)){
                    self.view.columnFilters[name].push(text);
                }
            });
        ...
        filter: function (collection, options) {
            var keys = _.keys(options), option,
                ...
                columnFilters = this.view.columnFilters;
                ...
                collection.reset(_.filter(collection.models, function (model) {
                    ...
                    if (option !== undefined) {
                        if (filterType === 'select') {
                            ...
                            //Move to other location where only iterates once.
                            _.each(columnFilters[key], function(el){
                                lowercaseFilters.push(el.toLowerCase())
                            });

                            return  _.contains(lowercaseFilters, ("" + option).toLowerCase());
                        } else {
        ...
        render: function (collection) {
            console.log(this.view.filterOptions);
            var self = this, filterBarHtml;
            _.each(this.view.colModel, function (col) {
                if (col.filter) {
                    filterCollection = new self.view._collection.constructor(self.view._collection.models);
                    //console.log(collection);
                    if(filterCollection != undefined ){
                        self.options[col.name] = _.uniq(filterCollection.pluck(col.filterColName || col.name));
                    }
            ...
            filterBarHtml = this.template({
                ...
                columnFilters: self.view.columnFilters
            });
